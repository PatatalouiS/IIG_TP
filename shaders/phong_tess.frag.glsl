...
#version 410 core
in vec3 normal_final;
in vec3 worldPosition_final;
in vec3 position;
...
void main() {
	...
	vec3 norm_ = normal_final;
	vec3 dir_key_light = normalize(pos_key_light - worldPosition_final);
	vec3 dir_fill_light = normalize(pos_fill_light - worldPosition_final);
	vec3 dir_back_light = normalize(pos_back_light - worldPosition_final);
	...
	vec3 main_color = vec3(normalize(normal_final)*0.5+0.5);
